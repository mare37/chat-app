  ...
  const response = await new Promise((resolve, reject) => {
    try {
      const response = axios
        .get(
          `http://localhost:5000/api/chatroom_requests/${userid}/${chatroomId}`
        )
        .then((object) => {
...
const getChatRoomRequests = async (chatroomId: number) => {
  const response = await axios.get(
    `http://localhost:5000/api/chatroom_requests/${chatroomId}`
  );
  //console.log(response);
...
const rejectRequest = async (userId: number, chatroomId: number) => {
  ...
  const response = await axios.delete(
    `http://localhost:5000/api/chatroom_requests/${userId}/${chatroomId}/reject`
  );
